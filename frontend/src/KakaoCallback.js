    ...
    if (code) {
      ...
      axios.post('https://kauth.kakao.com/oauth/token', null, {
        ...
        .then(userResponse => {
          ...
          const defaultNickname = `사용자_${idSuffix}`;
        ...
        .then(response => {
          ...
          // 기존 닉네임이 있으면 유지, 없으면 기본 닉네임 설정
          let nicknameToSet = `사용자_${userId.toString().slice(-3)}`;
          if (existingUserData && existingUserData.userId === userId && existingUserData.nickname) {
            nicknameToSet = existingUserData.nickname;
          }
